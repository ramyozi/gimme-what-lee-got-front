...
export interface SWRMultiSelectProps<T> {
  url: string;
  ...
  name?: string;
  onChange?: (selectedItemsIds: string[]) => void;
  filters?: Filter[];
  placeholder?: string;
  nothingFoundMessage?: string;
}

export function SWRMultiSelect<T extends Record<string, any>>({
  url,
  selectedItemsIds = [],
  ...
  const handleChange = useCallback(
    (newValues: string[]) => {
      if (formHook && name) {
        formHook.setFieldValue(name, newValues);
      } else if (onChange) {
        onChange(newValues);
      }
    },
  ...
  const currentValue =
    (formHook && name ? (formHook.getValues()[name] as string[]) : selectedItemsIds) ?? [];

  const safeValue = currentValue.filter((id) =>
